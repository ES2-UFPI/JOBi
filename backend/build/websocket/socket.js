    ...
    socket.on('iniciar_chat', async(params, callback) => {
        ...
        try{
            if(params.id_p != null && params.id_c != null){
               const conexao = await _Conexao2.default.findOne({ where: { prestador_id: params.id_p, contratante_id: params.id_c }});
               console.log(conexao)
                if(conexao){
                    socket.join(conexao.id);
                }else{
                   const new_conexao = await _Conexao2.default.create({
               ...
               const conexao = await _Conexao2.default.findOne({ where: { prestador_id: params.id_p, contratante_id: params.id_c }});
                if(conexao){
                    if(params.isPrest == false){
                        ...
                        await _Mensagem2.default.create({
