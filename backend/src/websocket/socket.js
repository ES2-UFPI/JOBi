...
io.on('connection', (socket) => {
    ...
    socket.on('iniciar_chat', async(params, callback) => {
        ...
        //socket.join(new_conexao.id)
         //callback devolve erro o a conexao feita
    ...
    socket.on('enviar_mensagem', async (params, callback) => {
        ...
        //Se quem enviou foi o contratante(params.isPrest=false) fazer:
        //socket.to(conexao.id).emit('message', {user: params.id_c, text: params.texto});
        //Inserir mensagem no banco
        //OU
        //Se quem enviou foi o prestador(params.isPrest=true) fazer:
        //socket.to(conexao.id).emit('message', {user: params.id_p, text: params.texto});
        //Criar mensagem de entrada ao prestador
        //Inserir mensagem no banco

         //callback devolve erro o a se tiver
    })
